...
OLLAMA_CLOUD_MODEL = os.getenv("OLLAMA_CLOUD_MODEL", "gpt-oss:20b-cloud")
...
ollama_cloud_client = OpenAI(base_url="https://ollama.com/v1", api_key=OLLAMA_API_KEY)
...
def call_ollama_cloud():
    ollama_cloud_messages = [m for m in messages if m["role"] != "system"]

    response = ollama_cloud_client.chat.completions.create(
        model=OLLAMA_CLOUD_MODEL,
        messages=[{"role": "system", "content": system_prompt}] + ollama_cloud_messages,
        tools=openai_tools,
        tool_choice="auto"  # Cloud models are more reliable, "auto" should work
    )
    message = response.choices[0].message
    ...
    if message.tool_calls:
        ...
        messages.clear()
        messages.append({"role": "system", "content": system_prompt})
        messages.extend(ollama_cloud_messages)
...
def run_agent():
    ...
    while True:
        ...
        if API_PROVIDER == "anthropic":
            ...
            reply = call_ollama()
        elif API_PROVIDER == "ollama_cloud": 
            reply = call_ollama_cloud()
        else:
            reply = call_openai()
